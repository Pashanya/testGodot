...
func handle_wall_jump():
	...
	if Input.is_action_just_pressed("ui_jump"):
		velocity.x = wall_normal.x * 1.5 * movement_data.speed
		velocity.y = movement_data.jump_velocity
		just_wall_jumped = true
#	if Input.is_action_just_pressed("ui_jump") and wall_normal == Vector2.RIGHT:
...
func handle_jump():
	...
	if is_on_floor() or coyote_jump_timer.time_left > 0.0:
		...
		if Input.is_action_just_pressed("ui_jump") and air_jump and not just_wall_jumped:
...
func unpdate_animation(input_axis):
	if input_axis != 0:
